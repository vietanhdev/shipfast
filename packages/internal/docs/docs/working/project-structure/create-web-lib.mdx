...
ShipFast.dev uses [nx](https://nx.dev/) to manage the monorepo, `pnpm` workspaces and TypeScript paths to link to
...
A new webapp library needs a few different files, so ShipFast.dev comes with a generator to make the job easier.
