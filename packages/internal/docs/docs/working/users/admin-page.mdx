...
Access policies are used in the ShipFast.dev to restrict access to queries and mutations.
...
:::info
Access policies in the ShipFast.dev are implemented using the [Django REST - Access Policy](https://rsinger86.github.io/drf-access-policy/) library.
...
:::caution
By default, the ShipFast.dev uses the `IsAuthenticated` permission class from Django REST Framework to restrict access to API endpoints.
