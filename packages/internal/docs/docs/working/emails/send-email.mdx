...
In the ShipFast.dev, you can send emails from the backend by creating a new class that subclasses the `Email` class.
...
In the ShipFast.dev, email data is serialized using the Django REST Framework serializers.
...
In the ShipFast.dev, the `Email` class is a subclass of the `common.tasks.Task` class, which emits the email event to be handled by async workers.
