...
```shell
pnpm saas aws set-var SHIPFAST_HOSTED_ZONE_ID XYZ
pnpm saas aws set-var SHIPFAST_HOSTED_ZONE_NAME example.com
pnpm saas aws set-var SHIPFAST_DOMAIN_ADMIN_PANEL admin.qa.example.com
pnpm saas aws set-var SHIPFAST_DOMAIN_API api.qa.example.com
pnpm saas aws set-var SHIPFAST_DOMAIN_CDN cdn.qa.example.com
pnpm saas aws set-var SHIPFAST_DOMAIN_DOCS docs.qa.example.com
pnpm saas aws set-var SHIPFAST_DOMAIN_WEB_APP app.qa.example.com
```

:::info
`SHIPFAST_HOSTED_ZONE_ID` and `SHIPFAST_HOSTED_ZONE_NAME` can be skipped if you are using externally managed DNS. In that case,
certificates for CloudFront distribution and Load Balancer should be already generated, and their ARNs provided in
`SHIPFAST_CLOUDFRONT_CERTIFICATE_ARN` and `SHIPFAST_LOAD_BALANCER_CERTIFICATE_ARN` parameters. As the last step,
...
ShipFast.dev creates a certificate that supports `*.[ENV_STAGE].example.com` domains, like
...
```
pnpm saas aws set-var SHIPFAST_CERTIFICATE_DOMAIN example.com
```
...
```shell
pnpm saas aws set-var SHIPFAST_BASIC_AUTH username:password
```
...
`SHIPFAST_DEPLOY_BRANCHES` variable.

```shell
pnpm saas aws set-var SHIPFAST_DEPLOY_BRANCHES master
```
...
```shell
pnpm saas aws set-var SHIPFAST_TOOLS_ENABLED <true/false>
pnpm saas aws set-var SHIPFAST_TOOLS_BASIC_AUTH username:password
pnpm saas aws set-var SHIPFAST_TOOLS_HOSTED_ZONE_ID XYZ
pnpm saas aws set-var SHIPFAST_TOOLS_HOSTED_ZONE_NAME example.com
pnpm saas aws set-var SHIPFAST_TOOLS_DOMAIN_VERSION_MATRIX status.example.com
```
